var path = require('path'), processPath = path.dirname(process.argv[1]);// 运行node的目录，这里可以方便替换下面baseDir的__dirname,方便用户自己搭建目录，利用node_modules加载rrestjs
...
module.exports = {
	...
	baseDir : path.join(__dirname, '/..'), // 绝对目录地址，下面的目录配置都是根据这个目录的相对地址，这里是根据config文件进行配置地址  baseDir:processPath,//这里是根据启动nodejs的命令目录来设置baseDir
	...
	ClusterNum : 1, // 开启的进程数
	ClusterReload : '/controller',// 只有当进程数为1时，进入开发模式，可以监听此文件夹下的改动，包括子文件夹，不用重复
	// ctrl+c 和 上键+enter
	...
	ZlibArray : [ 'text/plain', 'application/javascript', 'text/css',
			'application/xml', 'text/html' ], // 只压缩数组中的content-type响应
	...
	MongodbDefaultDbName : 'iArduinoWo',// 默认使用的数据库名
